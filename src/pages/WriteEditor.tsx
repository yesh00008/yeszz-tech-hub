...
import {
  ...
  ArrowLeft, Loader2, Code, FileCode2, Layers
} from "lucide-react";
...
const WriteEditor = () => {
  ...
  const [editorMode, setEditorMode] = useState<"monaco" | "simple">("monaco");
  ...
  useEffect(() => {
    ...
    if (id) {
      supabase.from("posts").select("*").eq("id", id).eq("author_id", user.id).single().then(({ data }) => {
        if (data) {
          // Parse CSS/JS from content if stored together
          const parsed = parseContentParts(data.content || "");
          setPost({
            title: data.title || "",
            slug: data.slug || "",
  ...
  // Parse stored content back into HTML/CSS/JS parts
  const parseContentParts = (content: string) => {
    let html = content;
    let css = "";
    let js = "";
    ...
    if (styleMatch) {
      styleMatch.forEach(block => {
        css += block.replace(/<\/?style[^>]*>/gi, "").trim() + "\n";
    ...
    if (scriptMatch) {
      scriptMatch.forEach(block => {
        js += block.replace(/<\/?script[^>]*>/gi, "").trim() + "\n";
        html = html.replace(block, "");
      });
    }
  ...
  // Combine HTML/CSS/JS into a single content string for storage
  const buildFullContent = useCallback(() => {
    let full = post.content;
    if (post.css_content.trim()) {
      full += `\n<style>\n${post.css_content}\n</style>`;
    }
    if (post.js_content.trim()) {
      full += `\n<script>\n${post.js_content}\n</script>`;
    }
    return full;
  }, [post.content, post.css_content, post.js_content]);
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <div className="inline-flex items-center rounded-lg border border-border bg-secondary/50 p-0.5">
                  <button
                    ...
                    className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
                      editorMode === "monaco" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground"
                    }`}
                  >
                    <Code className="h-3.5 w-3.5" /> Code Editor
                  </button>
                  <button
                    onClick={() => setEditorMode("simple")}
                    className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
                      editorMode === "simple" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground"
                    }`}
                  >
                    <FileCode2 className="h-3.5 w-3.5" /> Simple
                  </button>
                </div>
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                    showAI ? "bg-primary text-primary-foreground" : "border border-border text-muted-foreground hover:text-foreground"
                  }`}
                >
                ...
                >
                  <div className="rounded-xl border border-border bg-primary/5 p-4 space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button onClick={handleAIImprove} disabled={aiLoading} className="text-xs rounded-full border border-border px-3 py-1 hover:bg-secondary transition-colors disabled:opacity-50">✨ Improve</button>
                      <button onClick={handleAISummary} disabled={aiLoading} className="text-xs rounded-full border border-border px-3 py-1 hover:bg-secondary transition-colors disabled:opacity-50">📝 Summary</button>
                      <button onClick={handleAISEO} disabled={aiLoading} className="text-xs rounded-full border border-border px-3 py-1 hover:bg-secondary transition-colors disabled:opacity-50">🔍 SEO</button>
                    </div>
            ...
            <div className="grid lg:grid-cols-[1fr_280px] gap-6">
              ...
              <div className="space-y-4">
                ...
                {editorMode === "monaco" ? (
                  <MonacoCodeEditor
                    html={post.content}
                    css={post.css_content}
                    js={post.js_content}
                    onHtmlChange={(v) => setPost(p => ({ ...p, content: v }))}
                    onCssChange={(v) => setPost(p => ({ ...p, css_content: v }))}
                    onJsChange={(v) => setPost(p => ({ ...p, js_content: v }))}
                  />
                ) : (
              ...
              <div className="space-y-4">
                <div className="rounded-xl border border-border bg-card p-4">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <input type="text" value={post.slug} onChange={(e) => setPost(p => ({ ...p, slug: e.target.value }))} className="w-full rounded-lg border border-input bg-secondary px-3 py-2 text-sm outline-none focus:border-primary transition-all" />
                    </div>
                    <div>
                      <label className="text-xs font-medium text-muted-foreground mb-1 block">Category</label>
                      <select value={post.category_id} onChange={(e) => setPost(p => ({ ...p, category_id: e.target.value }))} className="w-full rounded-lg border border-input bg-secondary px-3 py-2 text-sm outline-none focus:border-primary transition-all">
                    ...
                    <div>
                      ...
                      <input type="text" value={post.read_time} onChange={(e) => setPost(p => ({ ...p, read_time: e.target.value }))} className="w-full rounded-lg border border-input bg-secondary px-3 py-2 text-sm outline-none focus:border-primary transition-all" />
                    </div>
                    <div>
                      <label className="text-xs font-medium text-muted-foreground mb-1 block">Cover Image URL</label>
                      <input type="url" value={post.image_url} onChange={(e) => setPost(p => ({ ...p, image_url: e.target.value }))} placeholder="https://..." className="w-full rounded-lg border border-input bg-secondary px-3 py-2 text-sm outline-none focus:border-primary transition-all" />
                      {post.image_url && <img src={post.image_url} alt="Cover" className="mt-2 rounded-lg w-full aspect-video object-cover" />}
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-xl border border-border bg-card p-4">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <textarea value={post.meta_description} onChange={(e) => setPost(p => ({ ...p, meta_description: e.target.value }))} rows={3} placeholder="SEO description (under 160 chars)" className="w-full rounded-lg border border-input bg-secondary px-3 py-2 text-sm outline-none focus:border-primary transition-all resize-none" />
                    ...
                    <div>
                      ...
                      <input type="text" value={post.meta_keywords} onChange={(e) => setPost(p => ({ ...p, meta_keywords: e.target.value }))} placeholder="react, ai, web" className="w-full rounded-lg border border-input bg-secondary px-3 py-2 text-sm outline-none focus:border-primary transition-all" />
                    </div>
                ...
                <div className="rounded-xl border border-border bg-card p-4">
                  <h3 className="text-sm font-semibold mb-3 flex items-center gap-2">
                    <Layers className="h-4 w-4 text-muted-foreground" /> Content Stats
                  </h3>
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="rounded-lg bg-secondary p-2 text-center">
                      <p className="font-bold text-foreground">{post.content.length}</p>
                      <p className="text-muted-foreground">HTML chars</p>
                    </div>
                    <div className="rounded-lg bg-secondary p-2 text-center">
                      <p className="font-bold text-foreground">{post.css_content.length}</p>
                      <p className="text-muted-foreground">CSS chars</p>
                    </div>
                    <div className="rounded-lg bg-secondary p-2 text-center">
                      <p className="font-bold text-foreground">{post.js_content.length}</p>
                      <p className="text-muted-foreground">JS chars</p>
                    </div>
                    <div className="rounded-lg bg-secondary p-2 text-center">
                      <p className="font-bold text-foreground">{post.content.split(/\s+/).filter(Boolean).length}</p>
