-- =============================================
-- YESZZ TECH — Creator Features (Tags, Tips, SEO)
-- =============================================
...
CREATE POLICY "Authenticated users can create tags" ON public.tags 
  FOR INSERT TO authenticated WITH CHECK (length(name) > 0 AND length(slug) > 0);
...
CREATE POLICY "Tips are viewable by creator" ON public.creator_tips 
  FOR SELECT TO authenticated USING (creator_id = auth.uid());
CREATE POLICY "Authenticated users can tip" ON public.creator_tips 
  FOR INSERT TO authenticated WITH CHECK (tipper_id = auth.uid());
